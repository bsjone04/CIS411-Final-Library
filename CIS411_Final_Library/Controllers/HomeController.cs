...
namespace CIS411_Final_Library.Controllers
{
    public class HomeController : Controller
    {
        ...
        public JsonResult SendRating(string r, string id, string url)
        {
            ...
            var isIt = db.SchoolModels.Where(v => v.UserName.Equals(User.Identity.Name, StringComparison.CurrentCultureIgnoreCase) && v.VoteForId == autoId).FirstOrDefault();
            if (isIt != null)
            {
            ...
            var sch = db.Books.Where(sc => sc.BookID == autoId).FirstOrDefault();
            if (sch != null)
            {
                ...
                SchoolModel vm = new SchoolModel()
                {
                    Active = true,
